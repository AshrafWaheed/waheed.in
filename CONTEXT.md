...
### Session 14 — May 2026
- Hero section tweaks applied: H1 font-size reduced to clamp(2.8rem, 4.5vw, 4.2rem). Subheadline now 1.125rem / lineHeight 1.75 / max-w-xl. Tag line changed from "— India's First Halal Digital Studio" (uppercase) to "A studio aligned with your values" (italic, clean). Scroll indicator block removed entirely. Floating service cards restructured — wrapper div handles CSS float (translateY) + marginLeft offsets (0/40/10px) and staggered floatDelay (0/1.5/3s); inner motion.div handles Framer entrance only, eliminating transform conflict. CSS keyframes consolidated to single @keyframes floatCard. Global body text audit: HomePhilosophy, HomeProcess, HomeServices desc/tagline raised from text-sm to text-base; HomeBlog excerpt raised from text-xs to text-sm; HomeNewsletter body raised from text-sm to text-base. Clean build, HTTP 200.
